		...
		if (iResult > 0) {
			...
			if (request == "LIST") {
				...
				while (getline(fp, buffer)) {
					buffer += "\n";
					iResult = send(clientSocket, buffer.c_str(), buffer.size(), 0);
				...
				string termination_message = "END_LIST";
				iResult = send(clientSocket, termination_message.c_str(), termination_message.size(), 0);
				if (iResult == SOCKET_ERROR) {
					cout << "send failed: " << WSAGetLastError() << endl;
					closesocket(clientSocket);
					WSACleanup();
